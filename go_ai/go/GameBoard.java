...
public class GameBoard extends JPanel {
    ...
    public GameBoard() {
        ...
        decision_AI = new Decision();
        decision_AI.init_AI(SIZE);
        ...
        this.addMouseListener(new MouseAdapter() {
            ...
            public void mouseReleased(MouseEvent e) {
                ...
                while(true) {
                    int nextMove = decision_AI.placeMove_AI(row * SIZE + col);
                    //System.out.println("lol2\n");
                    System.out.println(nextMove);

                    int row_n = nextMove % SIZE;
                    int col_n = nextMove / SIZE;
                    if(grid.isOccupied(row_n, col_n)) continue;
                    if(grid.addStone(row_n, col_n, current_player) == 1) {
                        lastMove = new Point(col_n, row_n);
                        if (current_player == State.BLACK) {
                            current_player = State.WHITE;
                        } else {
                            current_player = State.BLACK;
